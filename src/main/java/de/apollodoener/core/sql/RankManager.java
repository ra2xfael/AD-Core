    ...
    if (args.length == 2) {
      switch (args[0]) {
        case "get":
          ...
          if (player == null) {
            sender.sendMessage(ApolloDoener.getConfig(ConfigType.MESSAGES).getMessage("playerNotFound").replace("%player%", args[1]));
            return false;
          }

          sender.sendMessage(ApolloDoener.getConfig(ConfigType.MESSAGES).getMessage("rank.design").replace("%player%", player.getName()));
          if (getSince(player.getUniqueId()) == 0) {
            sender.sendMessage(ApolloDoener.getConfig(ConfigType.MESSAGES).getMessage("rank.rank").replace("%rank%", getRank(player.getUniqueId())));
            return false;
          }
          ...
          sender.sendMessage(ApolloDoener.getConfig(ConfigType.MESSAGES).getMessage("rank.rank").replace("%rank%", getRank(player.getUniqueId())));
          sender.sendMessage(ApolloDoener.getConfig(ConfigType.MESSAGES).getMessage("rank.since").replace("%time%", time));

          break;
        default:
          sender.sendMessage(ApolloDoener.getConfig(ConfigType.MESSAGES).getMessage("rank.syntax"));
          break;
      }
    } else if ((args.length == 3) && args[0].equalsIgnoreCase("promote")) {
      Player player = CommandUtil.getPlayer(sender, args[1]);
      if (player == null) {
        sender.sendMessage(ApolloDoener.getConfig(ConfigType.MESSAGES).getMessage("playerNotFound").replace("%player%", args[2]));
        return false;
      }
      String rank = args[2];
      if (addRank(player.getUniqueId(), rank))
        sender.sendMessage(ApolloDoener.getConfig(ConfigType.MESSAGES).getMessage("rank.promoted").replace("%player%", player.getName()));
      else
        sender.sendMessage(ApolloDoener.getConfig(ConfigType.MESSAGES).getMessage("sqlException"));
    } else {
      sender.sendMessage(ApolloDoener.getConfig(ConfigType.MESSAGES).getMessage("rank.syntax"));
      return true;
    }
